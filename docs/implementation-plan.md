...
export interface LLMMessage { role: Role; content: string | ContentBlock[] }
...
- **交付物**：
  - `glob` + `grep`（封装 ripgrep 二进制，无需 npm 重写）；
  - `write_file`、`replace_in_file` 严格唯一匹配 + `replaceAll`；
  - 权限门：写/执行类默认 `confirm`，可 `session`/`once` 放权；
  - 审计日志：每次工具决策落盘。
...
- **交付物**：
  ...
  - tree-sitter 符号表：`find_definition` `list_symbols` 工具；
  -（可选）向量检索：`semantic_search`（嵌入入库、sqlite-vec / hnswlib）；
  - 增量索引 + gitignore 过滤。
...
**MVP（对外可用）= Phase 1–3 合并体**，约 1.5–2 周。
...
| 工具副作用不可逆 | 误删/误执行 | 默认 confirm + `.kode/undo/` + 优先与 git 交互；破坏性写要求仓库干净 |
